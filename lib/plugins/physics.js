...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  // Movement packets only exist in the play state. Writing one while the client
  // has briefly re-entered the configuration state (a server switch / Velocity
  // transfer) serializes to a corrupt packet the server can't decode, which
  // kicks us ("internal error"). The vanilla client never sends movement
  // outside the play state.
  function inPlayState () {
    return bot._client.state === 'play'
  }
  ...
  function updatePosition (now) {
    ...
    } else if ((positionUpdateSentEveryTick || onGround !== lastSent.onGround) && inPlayState()) {
